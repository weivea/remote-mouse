...
## 运行 Windows
```powershell
.\rmserver.exe -pass 1234             # 默认打开「控制面板」+ 托盘（推荐，见下节）
.\rmserver.exe -pass 1234 -standalone # 旧版简易窗口（只读信息 + 设备列表）
.\rmserver.exe -pass 1234 -notray     # 纯后台，无窗口无托盘
```
关闭窗口（×）隐藏到托盘不退出；托盘左键切换显隐，右键菜单含 **显示窗口 / 登录时打开界面 / 退出**。SendInput 注入无需额外授权（已登录会话），支持鼠标/滚动/Unicode 文本/方向键/快捷键/媒体键。

## 控制面板（推荐）

单窗口集中所有操作：

- **连接信息**：推荐 IP（只读+复制）、端口/密码/设备名**可编辑**，「保存并应用」自动生效（服务运行则重启，约 1 秒；否则进程内热重配，已连接手机不掉线）。
- **服务管理**：实时状态 +「启动类型」下拉（自动/手动/禁用）+ 安装/启动/停止/卸载按钮（按状态启停用，操作弹 UAC）。
- **已连接设备**：独立模式实时、服务模式经状态管道显示（约 1 秒刷新）。
