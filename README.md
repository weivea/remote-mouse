...
> M1/M2 已落地，M3 部分完成：iOS(SwiftUI) ↔ Go 服务端，mDNS 发现 + 密码认证 + 鼠标/滚动/文本/键盘事件；mac 端到端可跑通，Windows SendInput 真机注入、可视化窗口/托盘 UI 与开机自启已完成。另有 M5 锁屏注入 POC（Windows SYSTEM 服务，iPhone 打 Hello PIN 解锁，待真机端到端验证）。设计文档见 [`docs/`](docs/README.md)。
...
- Windows 锁屏解锁（M5 POC）：需装成 SYSTEM 服务才能在锁屏/登录界面注入；安装步骤与**真机验证清单**见 [docs/09-lock-screen.md](docs/09-lock-screen.md)。
...
- 连上后默认进入触控板界面：拖动移光标、轻点左键、右侧条滚动。顶部 3 个悬浮按钮可切换「触控板 / 文本 / 全键盘」三个界面，支持横屏与竖屏。
